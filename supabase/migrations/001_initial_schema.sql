-- MU Gold Smith Master Database Schema
-- Run this script in the Supabase SQL Editor to initialize all tables, RLS policies, storage buckets, and seed data.
...
create table if not exists public.products (
  ...
  delivery_charges numeric(12, 2) not null default 200.00,
...
-- Orders Table (COD Storefront & Admin Orders)
...
create policy "Authenticated can update product images" on public.product_images for update to authenticated using (true) with check (true);
...
create policy "Authenticated can insert product variants" on public.product_variants for insert to authenticated with check (true);
create policy "Authenticated can update product variants" on public.product_variants for update to authenticated using (true) with check (true);
create policy "Authenticated can delete product variants" on public.product_variants for delete to authenticated using (true);

-- Orders Policies (ALLOWS CUSTOMERS TO PLACE ORDERS PUBLICLY)
...
create policy "Authenticated upload category images" on storage.objects for insert to authenticated with check (bucket_id = 'category-images');
create policy "Authenticated update category images" on storage.objects for update to authenticated using (bucket_id = 'category-images');
create policy "Authenticated delete category images" on storage.objects for delete to authenticated using (bucket_id = 'category-images');
...
insert into public.categories (id, name, slug, image_url, parent_category) values
  ...
  ('a1000001-0000-4000-8000-000000000008', 'Islamic Bracelets', 'islamic-bracelets', 'https://images.unsplash.com/photo-1611591437281-460bfbe1220a?w=800&q=80', 'Islamic Jewellery')
on conflict (slug) do nothing;

insert into public.products (id, title, slug, description, price, compare_at_price, delivery_charges, category_id, is_personalized, is_active, is_bestseller, stock_status) values
  ('b2000001-0000-4000-8000-000000000001', 'Birth Flower Signature Name Necklace', 'birth-flower-signature-name-necklace', 'Elegant personalized name necklace with birth flower charm. Crafted in premium plating — perfect for everyday wear or gifting.', 1699, 2199, 200, 'a1000001-0000-4000-8000-000000000001', true, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000002', 'Rainbow Kids Name Necklace', 'rainbow-kids-name-necklace', 'Colorful kids name necklace with playful charms. Soft chain, safe clasp, and custom name engraving.', 1999, null, 200, 'a1000001-0000-4000-8000-000000000001', true, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000003', 'Viral Tulip Bracelet', 'viral-tulip-bracelet', 'Delicate tulip charm bracelet — a bestselling gift for her. Available in gold and silver tones.', 1899, 2299, 200, 'a1000001-0000-4000-8000-000000000002', true, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000004', 'Custom Name Print Shawl', 'custom-name-print-shawl', 'Soft premium shawl with custom name print. Ideal for winter gifting and celebrations.', 2999, null, 250, 'a1000001-0000-4000-8000-000000000001', true, true, false, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000005', 'Arabic Stone Cufflinks', 'arabic-stone-cufflinks', 'Premium cufflinks with Arabic calligraphy stone inlay. A refined gift for him.', 1999, 2499, 200, 'a1000001-0000-4000-8000-000000000004', true, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000006', 'Translucent Urdu Name Cufflinks', 'translucent-urdu-name-cufflinks', 'Elegant translucent cufflinks featuring custom Urdu name calligraphy.', 1799, null, 200, 'a1000001-0000-4000-8000-000000000004', true, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000007', 'Customized Wallet with Name & Charm', 'customized-wallet-with-name-charm', 'Genuine-feel leather wallet with name engraving and charm. Available in classic colours.', 2299, 2799, 200, 'a1000001-0000-4000-8000-000000000005', true, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000008', 'Personalize Gift Set for Him', 'personalize-gift-set-for-him', 'Complete gift set: wallet, cufflinks, and keychain — all personalized. Perfect anniversary or birthday gift.', 5399, 6499, 300, 'a1000001-0000-4000-8000-000000000005', true, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000009', 'Turkish Style Black Onyx Ring', 'turkish-style-black-onyx-ring', 'Bold Ottoman-inspired ring with natural black onyx stone. Statement piece for men.', 9999, null, 200, 'a1000001-0000-4000-8000-000000000006', false, true, false, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000010', 'Exclusive 4 Qul Calligraphy Necklace', 'exclusive-4-qul-calligraphy-necklace', 'Beautiful Islamic calligraphy necklace featuring the Four Quls. Spiritual and elegant.', 2499, 2999, 200, 'a1000001-0000-4000-8000-000000000007', false, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000011', 'Ayat ul Kursi Cuff Bangle', 'ayat-ul-kursi-cuff-bangle', 'Unisex cuff bangle with Ayat ul Kursi calligraphy. Premium finish, adjustable fit.', 2199, null, 200, 'a1000001-0000-4000-8000-000000000008', false, true, true, 'in_stock'),
  ('b2000001-0000-4000-8000-000000000012', 'Calligraphy Bismillah Lapel Pin', 'calligraphy-bismillah-lapel-pin', 'Refined Bismillah lapel pin in gold or silver finish. Subtle faith-inspired accessory.', 1699, null, 200, 'a1000001-0000-4000-8000-000000000007', false, true, false, 'in_stock')
on conflict (slug) do nothing;
...
insert into public.product_images (product_id, image_url, sort_order) values
  ...
  ('b2000001-0000-4000-8000-000000000012', 'https://images.unsplash.com/photo-1535632066927-ab7c9ab60908?w=1000&q=80', 0);
